...
from flaskblog.forms import RegistrationForm, LoginForm, UpdateAccountForm, RecipeForm, EditProfileForm, AdminUserCreateForm, AdminUserUpdateForm
from flaskblog.models import User, Recipe
...
def save_picture(form_picture, picture_of):
	random_hex = secrets.token_hex(8)
	f_name, f_ext = os.path.splitext(form_picture.filename)
	picture_fn = random_hex + f_ext
	if picture_of == current_user:
		path = os.path.join(app.root_path, 'static/profile_pics/'+current_user.username)
	else:
		path = os.path.join(app.root_path, 'static/recipe_pics/')
	...
	if picture_of == current_user:
		...
		return os.path.join(picture_fn)
...
def following():
	...
	#		recipes.append(user.recipes)
	return render_template('following.html', title='Following', users=users)
...
def account():
	...
	if form.validate_on_submit():
		if form.picture.data:
			picture_file = save_picture(form.picture.data, current_user)
...
def user(username):
    ...
    page = request.args.get('page', 1, type=int)
    recipes = Recipe.query.filter_by(user_id=user.id).paginate(page, 20, False)
...
def new_recipe():
	...
	if form.validate_on_submit():
		...
		if form.picture.data:
			picture_file = save_picture(form.picture.data, recipe)
			recipe.image_file = picture_file

		db.session.commit()
		flash('Your recipe has been creared!', 'success')
		return redirect(url_for('home'))
...
def recipe(recipe_id):
	...
	return render_template('recipe.html', name=recipe.name, recipe=recipe)
...
def update_recipe(recipe_id):
	...
	return render_template('create_recipe.html', title='Update Recipe', form=form)
